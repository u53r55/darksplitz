...
def md5crack(hash):
    ...
    if req != '':
        ...
        print('[*] Cracking from lea.kz')
        try: req = requests.get('https://lea.kz/api/hash/' + hash, headers = user_agent, timeout = 5).text
        except: req = 'onyon'
        if '404 Not Found' not in req:
            j = json.loads(req)
            print('[+] Found : ' + j['password'])
        else:
